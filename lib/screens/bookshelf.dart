...
class _BookshelfScreenState extends State<BookshelfScreen>
  ...
  Future<void> _resumeGroup(BookGroup group) async {
    ...
    for (final s in widget.sourceManager.sources) {
      if (s.name == group.sourceName) { source = s; break; }
    }
    if (source == null || widget.playerState == null) return;

    // Browse folder to get all items
    try {
      final allItems = await source.browse(group.folderKey.isEmpty ? '/' : group.folderKey);
      final audioItems = allItems.where((i) => !i.isFolder).toList();
      final item = BookItem(name: latest!.title, path: latest.path);
      widget.playerState!.loadFromSource(item, source, allItems: audioItems, resumeMs: latest.positionMs);
    } catch (_) {}
  }
  ...
  Widget _placeholderCover(BookGroup group, ColorScheme cs) {
    ...
    return Container(
      ...
      child: Icon(
        progressRatio > 0 ? Icons.play_circle_filled : Icons.auto_stories_rounded,
        color: cs.primary,
  ...
  Widget _buildGroupList(List<BookGroup> groups, ColorScheme cs,
    ...
    if (groups.isEmpty) {
      return Center(
        child: Column(
          ...
          children: [
            Icon(Icons.library_books_outlined, size: 64, color: cs.onSurfaceVariant),
        ...
        return ListTile(
              ...
              if (group.author.isNotEmpty || group.narrator.isNotEmpty)
                Text(
                  [if (group.author.isNotEmpty) group.author,
                   if (group.narrator.isNotEmpty) '演播: ${group.narrator}'].join(' · '),
          ...
          trailing: PopupMenuButton<String>(
            onSelected: (v) {
              if (v == 'edit') {
                Navigator.push(context, MaterialPageRoute(
                  builder: (_) => MetaEditScreen(group: group, db: widget.bookDb),
                )).then((_) => setState(() {}));
              }
              if (v == 'fav') {
            ...
            itemBuilder: (_) => [
              PopupMenuItem(
                value: 'edit',
                child: Row(children: [const Icon(Icons.edit, size: 18), const SizedBox(width: 8), const Text('编辑元数据')]),
              ),
              PopupMenuItem(
                value: 'fav',
                child: Row(children: [
                  Icon(group.favorited ? Icons.star : Icons.star_border, size: 18),
